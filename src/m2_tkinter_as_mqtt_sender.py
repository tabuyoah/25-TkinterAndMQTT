...
def main():
    ...
    root.bind('<Up>', lambda event: mqtt_client.send_message('forward_key', [left_speed_entry.get(), right_speed_entry.get()]))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=4, column=0)
    left_button['command'] = lambda: mqtt_client.send_message('left_button', ['left button'])
    root.bind('<Left>', lambda event: mqtt_client.send_message('left_key', ['left key']))
